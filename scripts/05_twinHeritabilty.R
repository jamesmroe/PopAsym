...
saveres=1 #1/0
schaefer = 1 #1 = cortex-wide, 0 = cluster-wise
...
DF = read.csv("/Users/jamesroe/LCBC/Users/jamesroe/PHD_project/Paper3/data/demog_hcp.csv", stringsAsFactors = F, header = T, sep="")


#load in base data if cluster-wise. If cortex-wide dooes this within loop below
if (schaefer != 1) {
  ...
  b="/Users/jamesroe/LCBC/Users/jamesroe/WORKINGDIR"
...
for (j in 1:ll) {
  if (j==1) {
    ...
    LL_AE=0
    LL_CE=0
    a2 = c2 = e2 = V= pA = pC = LL_ACE = LL_CE = LLAE = 0
    pp=list()
  ...
  DataDZplus=mtrX[(lenMZ+1):nrow(mtr),]
  mtrMZ=mtr[1:lenMZ,]
...
if (saveres == 1) {
  if (schaefer != 1) {
    print("saving output")
    write.table(TABLE, here("results/heritability",paste0("clustersTwinACE",brainvar,".csv")),quote=F,row.names = F,col.names = T)
  } else {
    print("saving output")
    write.table(aceout, here("results/heritability",paste0("cortexTwinACE",brainvar,".csv")),quote=F,row.names = F,col.names = T)
  }
}
